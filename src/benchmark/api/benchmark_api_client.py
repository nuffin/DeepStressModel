...
class BenchmarkAPIClient:
    ...
    async def _make_request(
        ...
        if self.signature_manager:
            ...
            if self.use_nonce and self.current_nonce:
                signature = self.signature_manager.generate_signature(
                    data, f"{timestamp}:{self.current_nonce}"
                )
            else:
                signature = self.signature_manager.generate_signature(data, timestamp)

        # 构建请求数据
        request_data = {"data": data, "timestamp": timestamp, "signature": signature}
        ...
        # 构建请求头
        request_headers = {"Content-Type": "application/json", "X-Timestamp": timestamp}
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                async with self.session.request(
                    method, url, json=request_data, headers=request_headers
                ) as response:
                    # 检查响应状态
                    if response.status == 200:
                        # 记录原始响应内容
                        ...
                        try:
                            ...
                            logger.debug(
                                f"响应数据结构: {list(response_data.keys()) if isinstance(response_data, dict) else '非字典类型'}"
                            )
                            ...
                            if not isinstance(response_data, dict):
                                raise ValueError(
                                    f"响应数据格式错误，期望字典类型，实际类型: {type(response_data)}"
                                )
                        ...
                        if response.status == 400 and "无效的时间戳" in error_text:
                            ...
                            try:
                                ...
                                if "server_time" in error_data:
                                    ...
                                    self.timestamp_validator.update_time_offset(
                                        server_time
                                    )
                                    ...
                                    # 更新时间戳并重新生成签名
                                    timestamp = (
                                        self.timestamp_validator.generate_timestamp()
                                    )
                                    ...
                                    if self.signature_manager:
                                        if self.use_nonce and self.current_nonce:
                                            signature = self.signature_manager.generate_signature(
                                                data,
                                                f"{timestamp}:{self.current_nonce}",
                                            )
                                        else:
                                            signature = self.signature_manager.generate_signature(
                                                data, timestamp
                        ...
                        if attempt < retry_count:
                            ...
                            logger.info(
                                f"将在 {retry_delay} 秒后重试 ({attempt + 1}/{retry_count})"
                            )
    ...
    async def register_device(
        self, hardware_info: Dict[str, Any] = None, nickname: str = None
    ) -> Dict[str, Any]:
        """
        注册设备
    ...
    def _generate_signature(
        self, data: Dict[str, Any], timestamp: str, nonce: str
    ) -> str:
        """
        生成签名
    ...
    async def get_datasets(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 构建请求数据
            data = {"device_id": self.device_id, "session_token": self.session_token}
            ...
            if response.get("status") == "success":
                logger.info(
                    f"获取数据集列表成功，共 {len(response.get('data', []))} 个数据集"
                )
        ...
        try:
            ...
            async with self.session.get(download_url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"下载数据集文件失败: {response.status} - {error_text}"
                    )
                    raise Exception(
                        f"下载数据集文件失败: {response.status} - {error_text}"
                    )
    ...
    async def submit_result(
        self, result: Dict[str, Any], hardware_info: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        提交测试结果
        ...
        try:
            ...
            if hardware_info:
                ...
                data["hardware_fingerprint"] = self._generate_hardware_fingerprint(
                    hardware_info
                )
            ...
            if response.get("status") == "success":
                logger.info(
                    f"提交测试结果成功，结果ID: {response.get('data', {}).get('result_id')}"
                )
                return response.get("data", {})
            else:
    ...
    async def get_offline_package(
        self, dataset_id: Union[int, str] = 1
    ) -> Dict[str, Any]:
        """
        获取离线测试数据包
        ...
        try:
            ...
            logger.debug(
                f"准备获取离线包: dataset_id={dataset_id}, api_key={self.api_key[:4]}..."
            )
            ...
            try:
                async with self.session.request(
                    "POST", full_url, json=data, headers=headers
                ) as response:
                    if response.status == 200:
                        # 读取原始响应内容
                        raw_response = await response.text()
                        ...
                        try:
                            ...
                            logger.debug(
                                f"响应数据结构: {list(package_data.keys()) if isinstance(package_data, dict) else '非字典类型'}"
                            )

                            # 验证离线包数据
                            if not isinstance(package_data, dict):
                                raise ValueError(
                                    f"响应数据格式错误，期望字典类型，实际类型: {type(package_data)}"
                                )

                            # 验证包格式
                            package_format = package_data.get("metadata", {}).get(
                                "package_format"
                            )
                            if package_format not in ["3.0", "4.0"]:
                                logger.error(f"不支持的离线包格式: {package_format}")
                                raise ValueError(
                                    f"不支持的离线包格式，需要3.0或4.0版本"
                                )
                            ...
                            if missing_fields:
                                raise ValueError(
                                    f"离线包数据缺少必要字段: {', '.join(missing_fields)}"
                                )

                            logger.info(
                                f"获取离线包成功: 数据集ID={dataset_id}, 包格式版本={package_format}"
                            )
                            return package_data
                        except json.JSONDecodeError as e:
    ...
    async def save_offline_package(
        self, offline_package: Dict[str, Any], save_path: str
    ) -> str:
        """
        保存离线包到文件
        ...
        try:
            ...
            # 获取数据集ID
            dataset_id = offline_package.get("metadata", {}).get(
                "dataset_id", "unknown"
            )
            ...
            file_path = (
                save_dir
                / f"dsm_offline_package_{dataset_id}_{int(time.time() * 1000)}.json"
            )
    ...
    async def decrypt_offline_package(
        ...
        try:
            ...
            logger.debug(
                f"离线包数据结构: {list(package_data.keys()) if isinstance(package_data, dict) else '非字典类型'}"
            )
            ...
            logger.debug("开始解密私钥...")
            # 1. 使用API密钥解密私钥
            if not isinstance(encrypted_private_key, dict) or not all(
                k in encrypted_private_key for k in ["salt", "iv", "data"]
            ):
            ...
            encrypted_session_key = base64.b64decode(
                encrypted_dataset["encrypted_session_key"]
            )
            session_key = self._decrypt_with_private_key(
                encrypted_session_key, private_key_pem
            )
            if not session_key:
            ...
            logger.debug(
                f"数据集解密成功，数据结构: {list(dataset.keys()) if isinstance(dataset, dict) else '非字典类型'}"
            )
            ...
            calculated_signature = self._generate_offline_signature(
                dataset, timestamp, self.api_key
            )
    ...
    def _decrypt_with_derived_key(
        self, password: str, salt: str, iv: str, encrypted_data: str
    ) -> str:
        """
        使用PBKDF2派生密钥进行解密
        ...
        try:
            ...
            # 解密
            cipher = Cipher(
                algorithms.AES(key), modes.CBC(iv_bytes), backend=default_backend()
            )
                ...
                if "-----BEGIN" in decrypted_str and "-----END" in decrypted_str:
                    ...
                    if len(pem_lines) >= 2:  # 至少需要头部和尾部
                        ...
                        if "PRIVATE KEY" in header:
                            ...
                            try:
                                key_type = (
                                    header.split("BEGIN ")[1].split("-----")[0].strip()
                                )
                                logger.debug(f"提取到的密钥类型: {key_type}")
                            except:
                                key_type = "PRIVATE KEY"  # 默认私钥类型
                                logger.debug(
                                    f"无法提取密钥类型，使用默认类型: {key_type}"
                                )
                        ...
                        if pem_lines[0] != correct_header:
                            logger.debug(
                                f"修正头部: {pem_lines[0]} -> {correct_header}"
                            )
                        ...
                        if pem_lines[-1] != correct_footer:
                            logger.debug(
                                f"修正尾部: {pem_lines[-1]} -> {correct_footer}"
                            )
                        ...
                        # 验证PEM格式
                        if not (
                            corrected_pem.startswith("-----BEGIN")
                            and corrected_pem.endswith("-----")
                        ):
    ...
    def _decrypt_with_private_key(
        self, encrypted_data: bytes, private_key_pem: str
    ) -> bytes:
        """
        使用RSA私钥解密数据
        ...
        try:
                    ...
                    if "BEGIN" in header:
                        ...
                        if match:
                            ...
                            if "END" in footer and key_type not in footer:
                                ...
                                manual_pem = "\n".join(
                                    line.strip() for line in pem_lines
                                )
            ...
            if len(encrypted_data) > private_key.key_size // 8:
                logger.error(
                    f"加密数据长度({len(encrypted_data)})超过密钥大小({private_key.key_size // 8})"
                )
                return None
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"所有解密方法均失败: PKCS1v15: {str(e)}, OAEP: {str(e2)}"
                    )
                    return None
    ...
    def _decrypt_with_aes(
        self, encrypted_data: Dict[str, str], key: bytes
    ) -> Dict[str, Any]:
        """
        使用AES解密数据
        ...
        try:
            ...
            if len(key) != 16 and len(key) != 24 and len(key) != 32:
                logger.warning(
                    f"AES密钥长度({len(key)})不是标准长度(16/24/32)，进行调整"
                )
    ...
    def _generate_offline_signature(
        self, data: Dict[str, Any], timestamp: str, secret_key: str
    ) -> str:
        """
        生成离线包签名
