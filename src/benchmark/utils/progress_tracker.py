    ...
    def update_progress(self, progress_info: Dict[str, Any]):
        ...
        if self.callback:
            ...
            if total_duration > 0:
                ...
                avg_gen_speed = (
                    (total_chars / total_duration / concurrency)
                    if total_duration > 0
                    else 0
                )
            ...
            # 记录token信息
            logger.debug(
                f"Token信息 - 输入Token: {total_input_tokens}, 输出Token: {total_output_tokens}, 总Token: {total_input_tokens + total_output_tokens}"
            )
            ...
            combined_tps = (
                (total_input_tokens + total_output_tokens) / total_duration
                if total_duration > 0
                else 0
            )
            ...
            # 记录TPS计算日志
            logger.debug(
                f"实时TPS计算 - input_tps: {input_tps:.2f}, output_tps: {output_tps:.2f}, combined_tps: {combined_tps:.2f}"
            )

            # 获取状态统计
            status_counts = progress_info.get("status_counts", {})
            # 计算失败数量
            failed_count = status_counts.get("error", 0) + status_counts.get(
                "timeout", 0
            )
            ...
            formatted_progress["datasets"] = {
                dataset_name: {
                    ...
                    # 传递TPS相关字段
                    "tps": progress_info.get(
                        "token_throughput", 0
                    ),  # 使用token吞吐量作为TPS
                    "token_throughput": progress_info.get(
                        "token_throughput", 0
                    ),  # 显式传递token吞吐量
                    "input_tps": input_tps,  # 传递计算的输入TPS
                    "output_tps": output_tps,  # 传递计算的输出TPS
                    "combined_tps": combined_tps,  # 传递计算的综合TPS
                    "avg_tps": progress_info.get(
                        "token_throughput", 0
                    ),  # 设置avg_tps等同于token吞吐量
                    # 添加考虑并发数的TPS值 (每个实例的平均TPS)
                    "avg_tps_per_instance": (
                        progress_info.get("token_throughput", 0) / concurrency
                        if concurrency > 0
                        else 0
                    ),  # 平均每个实例的TPS
                    "input_tps_per_instance": (
                        progress_info.get("input_tps", 0) / concurrency
                        if concurrency > 0
                        else 0
                    ),  # 平均每个实例的输入TPS
                    "output_tps_per_instance": (
                        progress_info.get("output_tps", 0) / concurrency
                        if concurrency > 0
                        else 0
                    ),  # 平均每个实例的输出TPS
            ...
            # 记录TPS相关的详细日志
            logger.debug(
                f"TPS数据 - token_throughput: {progress_info.get('token_throughput', 0)}, "
    ...
    def complete_test(self, test_results=None):
        ...
        if test_results:
            ...
            if successful_tests > 0:
                avg_latency = (
                    sum(r.get("latency", 0) for r in test_results) / successful_tests
                )
                avg_throughput = (
                    sum(r.get("throughput", 0) for r in test_results) / successful_tests
                )
                ...
                input_tps = (
                    total_input_tokens / total_duration if total_duration > 0 else 0
                )
                output_tps = (
                    total_output_tokens / total_duration if total_duration > 0 else 0
                )
                ...
                # 计算每个实例的平均TPS (考虑并发数)
                avg_token_tps_per_instance = (
                    avg_token_tps / concurrency if concurrency > 0 else 0
                )
                input_tps_per_instance = (
                    input_tps / concurrency if concurrency > 0 else 0
                )
                output_tps_per_instance = (
                    output_tps / concurrency if concurrency > 0 else 0
                )

                logger.debug(
                    f"TPS计算 - avg_token_tps: {avg_token_tps}, input_tps: {input_tps}, output_tps: {output_tps}, combined_tps: {combined_tps}"
                )
                logger.debug(
                    f"每实例TPS计算 - avg_tps_per_instance: {avg_token_tps_per_instance}, input_tps_per_instance: {input_tps_per_instance}, output_tps_per_instance: {output_tps_per_instance}, 并发数: {concurrency}"
                )
            else:
                avg_latency = 0
                avg_throughput = 0
            ...
            # 如果没有测试结果，只更新状态
            self.update_progress(
                {"progress": 100, "status": "测试完成", "total_time": total_duration}
            )
