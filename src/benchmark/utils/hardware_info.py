...
def get_hardware_info() -> Dict[str, Any]:
    ...
    try:
        ...
        if gpu_stats:
            ...
            # 收集内存信息
            memory_info = (
                f"{gpu_stats.total_memory}GB"
                if hasattr(gpu_stats, "total_memory") and gpu_stats.total_memory > 0
                else "未知"
            )
            ...
            if hasattr(gpu_stats, "gpus") and gpu_stats.gpus:
                ...
                for i, gpu in enumerate(gpu_stats.gpus):
                    ...
                    gpus.append(gpu_str)
                    logger.debug(
                        f"GPU服务器GPU {i+1}: {gpu_str} (原始内存值: {memory_mb}MB)"
                    )
            ...
            hardware_info["memory"] = (
                f"{int(system_info['memory_info']['total'] / (1024*1024*1024))}GB"
            )
            hardware_info["system"] = (
                f"{system_info['os_info']['system']} {system_info['os_info']['release']}"
            )
            hardware_info["gpu"] = "未知"
            ...
            logger.warning(
                "使用本地系统信息作为备用，而非GPU服务器信息。这是不正确的，请检查GPU监控配置。"
            )
