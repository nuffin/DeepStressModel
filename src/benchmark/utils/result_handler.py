...
class ResultHandler:
    ...
    def save_result(self, result: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(
                f"[save_result] 开始保存测试结果，framework_info存在: {'framework_info' in result}"
            )
            if "framework_info" in result:
                logger.info(
                    f"[save_result] 输入的framework_info: {result['framework_info']}"
                )

            # 保存前先更新model字段，确保它使用model_info中的model_name
            if (
                "model_info" in result
                and isinstance(result["model_info"], dict)
                and "model_name" in result["model_info"]
            ):
                model_name = result["model_info"]["model_name"]
                logger.info(
                    f"[save_result] 从model_info.model_name更新顶级model字段: {model_name}"
                )
            ...
            if "results" in result and isinstance(result["results"], list):
                ...
                if truncated_count > 0:
                    logger.info(
                        f"已截断 {truncated_count} 个字段，测试项总数: {total_items}"
                    )
            ...
            with open(result_path, "w", encoding="utf-8") as f:
                ...
                logger.info(
                    f"[save_result] 保存前检查，framework_info存在: {'framework_info' in result}"
                )
                if "framework_info" in result:
                    logger.info(
                        f"[save_result] 保存前的framework_info: {result['framework_info']}"
                    )
    ...
    def update_result(self, result_path: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (
                "model_info" in updates
                and isinstance(updates["model_info"], dict)
                and "model_name" in updates["model_info"]
            ):
                model_name = updates["model_info"]["model_name"]
                logger.info(
                    f"[update_result] 从model_info.model_name更新顶级model字段: {model_name}"
                )
                result["model"] = model_name
    ...
    def save_encrypted_result(
        self, result: Dict[str, Any], api_key: str
    ) -> Tuple[str, str]:
        """
        加密并保存测试结果
        ...
        try:
            ...
            if (
                ...
                model_name = result["model_info"]["model_name"]
                logger.info(
                    f"[save_encrypted_result] 从model_info.model_name更新顶级model字段: {model_name}"
                )
                result["model"] = model_name

            # 检查framework_info
            logger.info(
                f"[save_encrypted_result] 开始加密保存，framework_info存在: {'framework_info' in result}"
            )
            if "framework_info" in result:
                logger.info(
                    f"[save_encrypted_result] 加密前framework_info: {result['framework_info']}"
                )
            else:
                logger.warning(
                    "[save_encrypted_result] 加密前结果中不存在framework_info"
                )
            ...
            if original_path and os.path.exists(original_path):
                logger.info(
                    f"[save_encrypted_result] 使用已存在的原始结果文件: {original_path}"
                )
                ...
                if "framework_info" in result and result["framework_info"]:
                    ...
                    logger.info(
                        f"[save_encrypted_result] 需要更新原始文件中的framework_info"
                    )

                if (
                    "model_info" in result
                    and result["model_info"]
                    and "model_name" in result["model_info"]
                ):
                    updates["model_info"] = result["model_info"]
                    updates["model"] = result["model_info"]["model_name"]
                    logger.info(
                        f"[save_encrypted_result] 需要更新原始文件中的model和model_info"
                    )
                ...
                try:
                    with open(original_path, "r", encoding="utf-8") as f:
                        ...
                        logger.info(
                            f"[save_encrypted_result] 读取现有文件成功，framework_info存在: {'framework_info' in result_to_encrypt}"
                        )
                except Exception as e:
                    logger.error(
                        f"[save_encrypted_result] 读取原始文件时出错: {str(e)}"
                    )
                    # 回退到使用内存中的结果
            ...
            if encrypted_dir and not os.path.exists(encrypted_dir):
                logger.info(
                    f"[save_encrypted_result] 创建加密文件目录: {encrypted_dir}"
                )
            ...
            if len(api_key) < 32:
                logger.error(
                    f"[save_encrypted_result] API密钥长度不足32字符 ({len(api_key)})"
                )
                return original_path, ""
            ...
            try:
                # 加密并保存结果
                logger.info(
                    f"[save_encrypted_result] 开始加密测试结果到: {encrypted_path}"
                )
                encrypted_path_result = encryptor.encrypt_and_save(
                    result_to_encrypt, encrypted_path, api_key
                )

                if not encrypted_path_result:
                    logger.error(
                        f"[save_encrypted_result] 加密测试结果失败，返回路径为空"
                    )
                    return original_path, ""
                ...
                if not os.path.exists(encrypted_path):
                    logger.error(
                        f"[save_encrypted_result] 加密文件没有被创建: {encrypted_path}"
                    )
                    return original_path, ""

                logger.info(
                    f"[save_encrypted_result] 测试结果已加密并保存到: {encrypted_path}"
                )
                return original_path, encrypted_path

            except Exception as e:
                logger.error(
                    f"[save_encrypted_result] 加密测试结果时发生错误: {str(e)}"
                )
                return original_path, ""
    ...
    def upload_encrypted_result(
        ...
        try:
            ...
            if (
                ...
                model_name = result["model_info"]["model_name"]
                logger.info(
                    f"[upload_encrypted_result] 从model_info.model_name更新顶级model字段: {model_name}"
                )
                result["model"] = model_name
                ...
                if metadata and isinstance(metadata, dict) and "model_name" in metadata:
                    ...
                    logger.info(
                        f"[upload_encrypted_result] 从model_info.model_name更新metadata.model_name: {model_name}"
                    )
            ...
            if "encrypted_path" in result and os.path.exists(result["encrypted_path"]):
                logger.info(
                    f"[upload_encrypted_result] 使用已存在的加密文件: {result['encrypted_path']}"
                )
                ...
                # 否则重新加密并保存结果
                logger.info(
                    f"[upload_encrypted_result] 未找到已有加密文件，开始加密保存"
                )
                original_path, encrypted_path = self.save_encrypted_result(
                    result, api_key
                )
            ...
            if not server_url:
                ...
                return {"status": "error", "message": "服务器URL为空，无法上传"}
            ...
            if metadata and isinstance(metadata, dict):
                ...
                logger.info(
                    f"[upload_encrypted_result] 元数据已转换为JSON字符串，长度: {len(metadata_json)}"
                )
            else:
                # 创建基本元数据
                basic_metadata = {
                ...
                upload_data["metadata"] = metadata_json
                logger.info(
                    f"[upload_encrypted_result] 已创建基本元数据: {basic_metadata}"
                )
            ...
            try:
                ...
                logger.info(
                    f"[upload_encrypted_result] 上传数据包含字段: {list(upload_data.keys())}"
                )

                # 上传
                response = requests.post(
                    server_url, data=upload_data, files=files, headers=headers
                )
                ...
                if response.status_code == 200:
                    ...
                    # 响应状态码不是200
                    logger.error(
                        f"服务器返回错误状态码: {response.status_code}, 响应: {response.text}"
                    )
                    return {
                        "status": "error",
            ...
            return {"status": "error", "message": f"上传测试结果失败: {str(e)}"}
