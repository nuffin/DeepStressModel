...
class PluginManager:
    ...
    def discover_plugins(self) -> List[str]:
        ...
        logger.info(
            f"发现 {len(discovered_plugins)} 个插件: {', '.join(discovered_plugins)}"
        )
    ...
    def load_all_plugins(self) -> Dict[str, bool]:
        ...
        logger.info(
            f"加载了 {sum(results.values())} 个插件，失败 {len(results) - sum(results.values())} 个"
        )
        return results
    ...
    def unload_all_plugins(self) -> Dict[str, bool]:
        ...
        logger.info(
            f"卸载了 {sum(results.values())} 个插件，失败 {len(results) - sum(results.values())} 个"
        )
        return results
    ...
    def get_enabled_plugins(self) -> Dict[str, BenchmarkPlugin]:
        ...
        """
        return {
            name: plugin for name, plugin in self.plugins.items() if plugin.is_enabled()
        }
