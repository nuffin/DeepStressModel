...
from src.benchmark.utils.test_execution.test_executor import (
    execute_test,
    calculate_metrics,
)
...
class BenchmarkManager:
    ...
    def __init__(self, config=None):
        ...
        self.server_url = self.config.get(
            "benchmark.server_url", "http://localhost:8083"
        )
        ...
        # 输出当前测试模式
        logger.info(
            f"当前测试模式: {self._test_mode} ({'联网模式' if self._test_mode == 0 else '离线模式'})"
        )

        # 设置结果目录
        self.result_dir = os.path.join(
            os.path.expanduser("~"), ".deepstressmodel", "benchmark_results"
        )
    ...
    async def initialize_async(self):
        ...
        try:
            ...
            logger.info(
                f"跑分管理器初始化完成，当前模式: {'联网模式' if self._test_mode == 0 else '离线模式'}"
            )
            return True
        except Exception as e:
    ...
    async def download_dataset(self, dataset_version: str) -> bool:
        ...
        try:
            ...
            dataset_path = os.path.join(
                self.datasets_dir, f"dataset_{dataset_version}.json"
            )
    ...
    def upload_dataset(self, file_path: str = None) -> bool:
        ...
        try:
            ...
            local_path = os.path.join(
                self.datasets_dir, f"{dataset_version}-{file_name}"
            )
    ...
    async def run_benchmark(
        ...
        if not hasattr(self, "dataset_updated") or not self.dataset_updated:
            error_msg = (
                "测试未开始：请先联网更新数据集或上传测试集，确保使用最新的测试数据"
            )
        ...
        if (
            not api_url
            and model_params
            and isinstance(model_params, dict)
            and "api_url" in model_params
        ):
        ...
        if dataset_info and isinstance(dataset_info, dict):
            if "名称" in dataset_info:
                ...
                "metadata" in dataset_info
                and "dataset_name" in dataset_info["metadata"]
            ):
                progress_tracker.set_dataset_name(
        ...
        try:
            ...
            # 设置测试参数
            model_config = {"model": model, "test_mode": test_mode}
            ...
            if isinstance(model_params, dict):
                ...
                logger.warning(
                    f"跳过model_params更新：不是字典类型，而是{type(model_params).__name__}"
                )
            ...
            if offline_info and isinstance(offline_info, dict):
                # 检查元数据中的版本信息
                if (
                    "metadata" in offline_info
                    and "dataset_version" in offline_info["metadata"]
                ):
            ...
            if dataset_version == "unknown" and isinstance(self.dataset_info, dict):
                if "version" in self.dataset_info:
                    ...
                    "metadata" in self.dataset_info
                    and "version" in self.dataset_info["metadata"]
                ):
            ...
            if offline_info and isinstance(offline_info, dict):
                if (
                    "metadata" in offline_info
                    and "dataset_name" in offline_info["metadata"]
                ):
            ...
            # 如果离线信息中没有找到名称，尝试从self.dataset_info中获取
                ...
                if "名称" in self.dataset_info:
                    ...
                    "metadata" in self.dataset_info
                    and "dataset_name" in self.dataset_info["metadata"]
                ):
            ...
            # 比较两种不同方式计算的平均延迟
            logger.info(
                f"直接计算的avg_latency: {avg_latency:.2f}s, metrics['latency']: {metrics['latency']:.2f}s"
            )
            ...
            char_token_ratio = {
                "input_ratio": (
                    total_input_chars
                    / sum(r.get("input_tokens", 0) for r in test_results)
                    if sum(r.get("input_tokens", 0) for r in test_results) > 0
                    else 0
                ),
                "output_ratio": (
                    total_output_chars
                    / sum(r.get("output_tokens", 0) for r in test_results)
                    if sum(r.get("output_tokens", 0) for r in test_results) > 0
                    else 0
                ),
            ...
            if successful_tests > 0:
                ...
                avg_token_tps = (
                    sum(token_throughputs) / len(token_throughputs)
                ...
                logger.info(
                    f"TPS计算：输入TPS={input_token_tps:.2f}, 输出TPS={output_token_tps:.2f}, 综合TPS={combined_token_tps:.2f}"
                )
            ...
            datasets = {
                dataset_name: {
                    ...
                    "avg_gen_speed": (
                        (total_chars / total_time / concurrency)
                        if total_time > 0
                        else 0
                    ),  # 真正的字符生成速度：总字符数/(总时间*并发数)
                    ...
                    "avg_tps_per_instance": (
                        combined_token_tps / concurrency if concurrency > 0 else 0
                    ),  # 平均每个实例的TPS (综合)
                    "input_tps_per_instance": (
                        input_token_tps / concurrency if concurrency > 0 else 0
                    ),  # 平均每个实例的输入TPS
                    "output_tps_per_instance": (
                        output_token_tps / concurrency if concurrency > 0 else 0
                    ),  # 平均每个实例的输出TPS
            ...
            final_result = {
                ...
                "avg_gen_speed": (
                    (total_chars / total_time / concurrency) if total_time > 0 else 0
                ),  # 真正的字符生成速度，考虑并发数
                "tps": tps,
    ...
    async def get_offline_package(self, dataset_id: str, save_path: str = None) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"成功获取离线包，数据结构: {list(offline_package.keys()) if offline_package else '空'}"
            )
            ...
            package_path = await self.api_client.save_offline_package(
                offline_package, save_path
            )
    ...
    async def load_offline_package(
        self, package_path: str, api_key: str = None
    ) -> bool:
        """
        加载并解密离线包
        ...
        try:
            ...
            if server_url:
                ...
                upload_result = result_handler.upload_encrypted_result(
                    result, api_key=api_key, server_url=server_url, metadata=metadata
                )

                if upload_result.get("status") == "success":
                    logger.info(
                        f"加密结果上传成功，ID: {upload_result.get('upload_id', 'unknown')}"
                    )
                    return {
                        "status": "success",
            ...
            if test_mode == 0:  # 联网模式
                ...
                if upload_result.get("status") == "success":
                    ...
                    return {
                        "status": "success",
                        "message": "测试结果上传成功",
                        "upload_id": upload_result.get("upload_result", {}).get(
                            "upload_id", "unknown"
                        ),
                        "ui_message": "上传成功",
