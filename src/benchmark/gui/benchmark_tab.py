                ...
                if "results" in result:
                    ...
                    if all_tasks:  # 确保有测试任务
                        ...
                        successful_tasks = sum(
                            1 for r in all_tasks if r.get("status") == "success"
                        )
                        ...
                        if successful_tasks == 0 and len(all_tasks) > 0:
                            ...
                            for task in all_tasks:
                                ...
                                if error_msg:
                                    error_messages[error_msg] = (
                                        error_messages.get(error_msg, 0) + 1
                                    )
                            ...
                            if error_info:
                                most_common_error = max(
                                    error_info.items(), key=lambda x: x[1]
                                )[0]
                            ...
                            if error_messages:
                                most_common_msg = max(
                                    error_messages.items(), key=lambda x: x[1]
                                )[0]
                                # 截断过长的错误消息
                            ...
                            # 检查是否是连接错误
                            if (
                                "ClientConnectorError" in most_common_error
                                or "Connection" in most_common_error
                            ):
