    ...
    def load_history(self):
        ...
        try:
            ...
            for filename in os.listdir(self.result_dir):
                if filename.startswith("benchmark_result_") and filename.endswith(
                    ".json"
                ):
            ...
            for i, result in enumerate(self.results):
                ...
                self.history_table.setItem(
                    i, 1, QTableWidgetItem(result.get("model", "未知"))
                )

                # 精度
                self.history_table.setItem(
                    i, 2, QTableWidgetItem(result.get("precision", "未知"))
                )

                # 指标
                metrics = result.get("metrics", {})
                self.history_table.setItem(
                    i, 3, QTableWidgetItem(f"{metrics.get('throughput', 0):.2f}")
                )
                self.history_table.setItem(
                    i, 4, QTableWidgetItem(f"{metrics.get('latency', 0):.2f}")
                )
                self.history_table.setItem(
                    i, 5, QTableWidgetItem(f"{result.get('total_duration', 0):.2f}")
                )
