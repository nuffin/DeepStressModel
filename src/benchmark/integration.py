...
class BenchmarkIntegration(QObject):
    ...
    def _on_progress_updated(self, progress: Dict[str, Any]):
        ...
        # 添加调试日志
        logger.debug(
            f"BenchmarkIntegration: 收到进度更新，准备发送信号. 数据键: {list(progress.keys() if isinstance(progress, dict) else ['非字典数据'])}"
        )
        ...
        if "datasets" in progress and progress["datasets"]:
            ...
            logger.debug(
                f"BenchmarkIntegration: 数据集 '{first_dataset_name}' 的总耗时字段: {duration_fields}"
            )
            ...
            if (
                all_missing
                and "completed" in first_dataset
                and "total" in first_dataset
            ):
                # 检查是否有单项测试的耗时数据可以累加
                if "items" in first_dataset and isinstance(
                    first_dataset["items"], list
                ):
                    total_duration = sum(
                        item.get("duration", 0)
                        for item in first_dataset["items"]
                        if isinstance(item, dict)
                    )
                    if total_duration > 0:
                        logger.debug(
                            f"BenchmarkIntegration: 为数据集 '{first_dataset_name}' 补充总耗时: {total_duration}"
                        )
    ...
    def register_device(self, nickname: str, callback: Callable[[bool, str], None]):
        ...
        worker.finished.connect(
            lambda result: callback(
                result, "设备注册成功" if result else "设备注册失败"
            )
        )
    ...
    def authenticate(self, callback: Callable[[bool, str], None]):
        ...
        worker.finished.connect(
            lambda result: callback(
                result, "设备认证成功" if result else "设备认证失败"
            )
        )
    ...
    def get_datasets(self, callback: Callable[[List[Dict[str, Any]], str], None]):
        ...
        # 连接信号
        worker.finished.connect(
            lambda datasets: callback(datasets, f"获取到 {len(datasets)} 个数据集")
        )
    ...
    def download_dataset(
        self, dataset_version: str, callback: Callable[[bool, str], None]
    ):
        """
        下载数据集
        ...
        worker.finished.connect(
            lambda result: callback(
                result, "数据集下载成功" if result else "数据集下载失败"
            )
        )
        ...
        async def run_benchmark_with_init():
            ...
            try:
                ...
                if not api_url:
                    ...
                    if (
                        isinstance(framework_config, dict)
                        and "api_url" in framework_config
                    ):
                ...
                logger.info(
                    f"开始测试，参数: model={model}, precision={precision}, concurrency={concurrency}, api_timeout={api_timeout}"
                )
    ...
    def _on_test_finished(self, result: Dict[str, Any]):
        ...
        # 添加调试日志，检查框架信息
        logger.info(
            f"[_on_test_finished] 测试完成，framework_info存在: {'framework_info' in result}"
        )
        if "framework_info" in result:
            logger.info(
                f"[_on_test_finished] 信号发送前的framework_info: {result['framework_info']}"
            )

        # 检查result对象属性
        logger.info(
            f"[_on_test_finished] result对象id: {id(result)}, 类型: {type(result).__name__}"
        )
    ...
    def enable_benchmark_module(self, callback: Callable[[bool, str], None]):
        ...
        worker.finished.connect(
            lambda result: callback(
                result, "跑分模块启用成功" if result else "跑分模块启用失败"
            )
        )
        worker.error.connect(
            lambda error: callback(False, f"跑分模块启用失败: {error}")
        )
    ...
    def disable_benchmark_module(self, callback: Callable[[bool, str], None]):
        ...
        worker.finished.connect(
            lambda result: callback(
                result, "跑分模块禁用成功" if result else "跑分模块禁用失败"
            )
        )
        worker.error.connect(
            lambda error: callback(False, f"跑分模块禁用失败: {error}")
        )
    ...
    def set_api_key(
        self, api_key: str, device_id: str = None, nickname: str = None
    ) -> bool:
        """
        设置API密钥
    ...
    def encrypt_result(self) -> Dict[str, Any]:
        ...
        try:
            # 检查是否有测试结果
            if (
                not hasattr(self.benchmark_manager, "latest_test_result")
                or self.benchmark_manager.latest_test_result is None
            ):
                return {
                    "status": "error",
                    "message": "没有可加密的测试结果",
            ...
            if "encrypted_path" in latest_result and os.path.exists(
                ...
                logger.info(
                    f"测试结果已有加密文件，直接使用: {latest_result['encrypted_path']}"
                )
                return {
                    "status": "success",
                    "message": "使用已存在的加密文件",
            ...
            logger.info(
                f"加密前检查latest_result中framework_info存在: {'framework_info' in latest_result}"
            )
            if "framework_info" in latest_result:
                logger.info(
                    f"加密前latest_result中的framework_info: {latest_result['framework_info']}"
                )
    ...
    def upload_result(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                "model_name": self.benchmark_manager.latest_test_result.get(
                    "model_name", "未知模型"
                ),
                "hardware_info": self.benchmark_manager.latest_test_result.get(
                    "hardware_info", {}
                ),
            ...
            # 检查是否已经有加密文件
            if (
                "encrypted_path" not in self.benchmark_manager.latest_test_result
                or not os.path.exists(
                    self.benchmark_manager.latest_test_result["encrypted_path"]
                )
            ):
                ...
                if encrypt_result.get("status") != "success":
                    logger.error(
                        f"无法加密测试结果: {encrypt_result.get('message', '未知错误')}"
                    )
                    return encrypt_result
            else:
                logger.info(
                    f"使用已有的加密文件: {self.benchmark_manager.latest_test_result['encrypted_path']}"
                )
            ...
            if upload_result.get("status") == "success":
                logger.info(
                    f"测试结果上传成功，ID: {upload_result.get('upload_id', 'unknown')}"
                )
                return {
                    "status": "success",
                    "message": "测试结果上传成功",
    ...
    def get_offline_package(self, dataset_id: int, callback=None):
        ...
        try:
            ...
            logger.debug(
                f"获取离线包 - API密钥状态: {'已配置' if api_key else '未配置'}"
            )
            if not api_key:
                if callback:
            ...
            async def _get_package():
                try:
                    ...
                    # 重置API客户端状态
                    if (
                        hasattr(self.benchmark_manager, "api_client")
                        and self.benchmark_manager.api_client
                    ):
                        logger.debug("获取离线包 - 重置API客户端状态")
                        # 确保API客户端重新创建会话
                        if (
                            hasattr(self.benchmark_manager.api_client, "session")
                            and self.benchmark_manager.api_client.session
                        ):
                    ...
                    package = await self.benchmark_manager.get_offline_package(
                        dataset_id
                    )
                    logger.debug(
                        f"获取离线包 - 服务器响应: {package if package else '空'}"
                    )

                    if package:
                        logger.info(
                            f"获取离线包成功 - 数据集大小: {len(str(package))} 字节"
                        )
                        if callback:
    ...
    def load_offline_package(
        self, package_path: str, callback: Callable[[bool, str], None] = None
    ):
        """
        加载并解密离线包
        ...
        try:
            ...
            async def load_offline_package_with_init():
                ...
                # 加载离线包
                logger.debug(
                    f"离线包加载 - 调用benchmark_manager.load_offline_package, API密钥: {api_key[:4]}..."
                )
                return await self.benchmark_manager.load_offline_package(
                    package_path, api_key
                )
            ...
            if callback:
                ...
                def on_finished(result):
                    ...
                    message = (
                        "离线包加载成功"
                        if success
                        else "离线包加载失败，请检查文件格式或API密钥是否正确"
                    )
