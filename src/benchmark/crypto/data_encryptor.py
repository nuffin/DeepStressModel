...
class DataEncryptor:
        ...
        try:
            ...
            if self.server_public_key:
                encrypted_session_key = CryptoUtils.rsa_encrypt(
                    session_key, self.server_public_key
                )

            # 生成用户签名
            user_signature = None
            if self.signature_manager:
                user_signature = self.signature_manager.generate_signature(
                    data, timestamp
                )
    ...
    def decrypt_data(
        self, encrypted_package: Dict[str, Any], private_key: bytes = None
    ) -> Dict[str, Any]:
        """
        解密数据
        ...
        try:
            ...
            if encrypted_session_key and private_key:
                session_key = CryptoUtils.rsa_decrypt(
            ...
            if self.signature_manager:
                ...
                if user_signature and timestamp:
                    if not self.signature_manager.verify_signature(
                        decrypted_data, timestamp, user_signature
                    ):
    ...
    def decrypt_dataset(
        ...
        try:
            ...
            if self.signature_manager:
                ...
                if user_signature and timestamp:
                    if not self.signature_manager.verify_signature(
                        dataset, timestamp, user_signature
                    ):
                        logger.warning("用户签名验证失败")
    ...
    def decrypt_result(
        ...
        try:
            ...
            if self.signature_manager:
                ...
                if user_signature and timestamp:
                    if not self.signature_manager.verify_signature(
                        result, timestamp, user_signature
                    ):
                        logger.warning("用户签名验证失败")
