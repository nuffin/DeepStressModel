...
class TestBenchmarkEncryption(unittest.TestCase):
    ...
    def test_encrypt_benchmark_log(self):
        """测试基准测试日志加密功能"""
        encrypted_package = self.encryptor.encrypt_benchmark_log(
            self.test_log_data, self.api_key
        )
    ...
    def test_encrypt_and_save(self):
        ...
        try:
            # 加密并保存
            self.encryptor.encrypt_and_save(
                self.test_log_data, output_path, self.api_key
            )
