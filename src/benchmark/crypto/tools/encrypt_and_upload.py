...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DeepStressModel 测试结果加密与上传工具"
    )
    parser.add_argument("input", help="测试结果文件路径", type=str)
    parser.add_argument(
        "-o", "--output", help="加密输出文件路径", type=str, default=None
    )
    ...
    try:
        ...
        if args.upload:
            ...
            upload_result = encryptor.encrypt_and_upload(
                result, api_key=args.api_key, server_url=args.server, metadata=metadata
            )
