...
class DatasetManager:
            ...
            if package_data:
                ...
                if formatted_dataset:
                    ...
                    for name, data in formatted_dataset.items():
                        ...
                        logger.info(
                            f"已添加离线数据集: {new_name}, 包含 {len(data)} 个提示"
                        )
    ...
    def get_offline_dataset_data(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if (
                self.raw_dataset
                and "data" in self.raw_dataset
                and isinstance(self.raw_dataset["data"], list)
            ):
                logger.info(
                    f"获取到离线数据集原始数据，共 {len(self.raw_dataset['data'])} 条"
                )
    ...
    def load_benchmark_dataset(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"基准测试数据集加载成功，包含 {len(dataset.get('data', []))} 条测试项"
            )
            return True
