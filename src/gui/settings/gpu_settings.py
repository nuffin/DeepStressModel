...
class ServerEditDialog(QDialog):
    ...
    def __init__(self, server_data=None, parent=None):
        ...
        self.setWindowTitle(
            self.tr("edit_server") if server_data else self.tr("add_server")
        )
        self.init_ui()
        if server_data:
    ...
    def test_connection(self):
        ...
        try:
            ...
            monitor.setup_monitor(
                data["host"], data["username"], data["password"], data["port"]
            )
            stats = monitor.get_stats()

            if stats:
                QMessageBox.information(
                    self, self.tr("success"), self.tr("test_connection_success")
                )
                logger.info(f"服务器连接测试成功: {data['host']}:{data['port']}")
            else:
                QMessageBox.warning(
                    self, self.tr("warning"), self.tr("test_connection_no_gpu")
                )
                logger.warning(f"服务器GPU信息获取失败: {data['host']}:{data['port']}")
        except Exception as e:
            logger.error(f"服务器连接测试失败: {e}")
            QMessageBox.critical(
                self, self.tr("error"), f"{self.tr('test_connection_failed')}: {e}"
            )
    ...
    def accept(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, self.tr("error"), f"{self.tr('save_server_failed')}: {e}"
            )
