...
class DatasetEditDialog(QDialog):
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle(
            self.tr("edit_dataset") if self.dataset_data else self.tr("add_dataset")
        )
        layout = QVBoxLayout()
    ...
    def get_dataset_data(self) -> dict:
        """获取数据集数据"""
        prompts = [
            p.strip() for p in self.prompts_edit.toPlainText().split("\n") if p.strip()
        ]
        return {
...
class DatasetSettingsWidget(QWidget):
            ...
            try:
                ...
                dataset = next(
                    (
                        d
                        for d in datasets
                        if d["name"] == self.dataset_list.item(row).text()
                    ),
                    None,
                )
                if dataset:
        ...
        try:
            ...
            dataset = next(
                (
                    d
                    for d in datasets
                    if d["name"] == self.dataset_list.item(current_row).text()
                ),
                None,
            )
            if dataset:
    ...
    def import_dataset(self):
        ...
        try:
            ...
            # 添加选择框
            combo = QComboBox()
            combo.addItems([self.tr("import_builtin"), self.tr("import_file")])
    ...
    def _import_builtin_dataset(self):
        ...
        try:
            ...
            if dialog.exec():
                ...
                dataset_data = {"name": dataset_name, "prompts": DATASETS[dataset_name]}
                if db_manager.add_dataset(dataset_data):
                    self.load_datasets()
                    self.dataset_updated.emit()
        ...
        try:
            ...
            if file_path:
                ...
                if prompts:
                    ...
                    dataset_data = {"name": dataset_name, "prompts": prompts}
                    if db_manager.add_dataset(dataset_data):
                        self.load_datasets()
                        self.dataset_updated.emit()
