...
class BenchmarkHistoryTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.history_table.setHorizontalHeaderLabels(
            ["测试时间", "数据集版本", "模型", "精度", "平均TPS", "平均延迟", "操作"]
        )
        ...
        for filename in result_files:
            ...
            try:
                ...
                self.history_table.setItem(
                    row, 1, QTableWidgetItem(result.get("dataset_version", ""))
                )

                # 模型
                self.history_table.setItem(
                    row, 2, QTableWidgetItem(result.get("model", ""))
                )

                # 精度
                self.history_table.setItem(
                    row, 3, QTableWidgetItem(result.get("precision", ""))
                )
    ...
    def _update_detail(self, result):
        """更新详情面板"""
        # 更新标题
        start_time = datetime.fromisoformat(result.get("start_time", "")).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def update_ui_text(self):
        ...
        if self.selected_result:
            start_time = datetime.fromisoformat(
                self.selected_result.get("start_time", "")
            ).strftime("%Y-%m-%d %H:%M:%S")
