...
class ResultsTab(QWidget):
    ...
    def _add_record_to_table(self, row: int, record: dict):
        ...
        try:
            ...
            # 设置成功率
            success_rate = (
                (record["successful_tasks"] / record["total_tasks"] * 100)
            ...
            self.result_table.setItem(
                row, 3, QTableWidgetItem(f"{record['avg_response_time']:.1f}s")
            )

            # 设置平均生成速度
            self.result_table.setItem(
                row, 4, QTableWidgetItem(f"{record['avg_generation_speed']:.1f}字/秒")
            )

            # 设置当前速度
            self.result_table.setItem(
                row, 5, QTableWidgetItem(f"{record['current_speed']:.1f}字/秒")
            )

            # 设置总字符数
            self.result_table.setItem(
                row, 6, QTableWidgetItem(str(record["total_chars"]))
            )

            # 设置平均TPS
            self.result_table.setItem(
                row, 7, QTableWidgetItem(f"{record['avg_tps']:.1f}")
            )

            # 设置总耗时
            self.result_table.setItem(
                row, 8, QTableWidgetItem(f"{record['total_time']:.1f}s")
            )
            ...
            # 设置并发数
            self.result_table.setItem(
                row, 10, QTableWidgetItem(str(record["concurrency"]))
            )
            ...
            log_btn.clicked.connect(
                lambda: self._view_log(
                    record.get("log_file", ""), record["session_name"]
                )
            )
            ...
            delete_btn.clicked.connect(
                lambda: self._delete_record(record["session_name"])
            )
    ...
    def _view_log(self, log_file: str, session_name: str):
        """查看日志文件"""
        logger.debug(
            f"尝试查看日志文件，会话: {session_name}, 日志文件路径: {log_file}"
        )

        if not log_file:
            logger.warning(f"会话 {session_name} 的日志文件路径为空")
            QMessageBox.warning(
                self, self.tr("warning"), f"未找到会话 {session_name} 的日志文件"
            )
            return
        ...
        dialog.setWindowTitle(
            self.tr("test_log_title").format(session_name=session_name)
        )
            ...
            try:
                ...
                if result:
                    ...
                    QMessageBox.information(
                        self, self.tr("success"), self.tr("record_deleted")
                    )
                else:
    ...
    def _export_records(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, self.tr("export_title"), "", "CSV文件 (*.csv)"
        )
    ...
    def _clear_logs(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if db_manager.clear_test_logs():
                QMessageBox.information(
                    self, self.tr("success"), self.tr("logs_cleared")
                )
            else:
    ...
    def _save_test_records(self):
        ...
        try:
            ...
            # 确保model_name字段存在
            if (
                "model_name" not in self.current_records
                and "model_config" in self.current_records
            ):
                self.current_records["model_name"] = self.current_records[
                    "model_config"
                ]["name"]
            ...
            log_file = os.path.join(
                log_dir, f"{self.current_records.get('test_task_id', 'unknown')}.log"
            )
            ...
            current_speed = self.current_records.get(
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                ...
                f.write("测试完成统计信息:\n")
                f.write(
                    f"测试ID: {self.current_records.get('test_task_id', 'unknown')}\n"
                )
                f.write(
                    f"会话名称: {self.current_records.get('session_name', 'unknown')}\n"
                )
                f.write(
                    f"模型名称: {self.current_records.get('model_name', 'unknown')}\n"
                )
                f.write(f"并发数: {self.current_records.get('concurrency', 0)}\n")
                f.write(
                    f"开始时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(self.current_records.get('start_time', 0)))}\n"
                )
                f.write(
                    f"结束时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(self.current_records.get('end_time', time.time())))}\n"
                )
                f.write(f"总耗时: {total_time:.2f}秒\n\n")

                # 写入数据集统计信息
                f.write("数据集统计信息:\n")
                for dataset_name, stats in self.current_records.get(
                    "datasets", {}
                ).items():
                    f.write(f"\n{dataset_name}:\n")
                    ...
                    f.write(
                        f"  成功率: {(stats.get('successful', 0) / stats.get('total', 1) * 100):.1f}%\n"
                    )
                    f.write(
                        f"  平均响应时间: {stats.get('avg_response_time', 0):.2f}秒\n"
                    )
                    f.write(
                        f"  平均生成速度: {stats.get('avg_generation_speed', 0):.1f}字/秒\n"
                    )
            ...
            if response.success:
                ...
                if response.error_msg:
                    self.error_text.append(
                        f"数据集 {dataset_name} 错误: {response.error_msg}"
                    )
    ...
    def update_ui_text(self):
        ...
        for child in self.findChildren(QPushButton):
            if child.text() == self.tr("export_records") or child.text().startswith(
                "导出"
            ):
                child.setText(self.tr("export_records"))
            elif child.text() == self.tr("clear_logs") or child.text().startswith(
                "清除"
            ):
