...
class GPUMonitorWidget(QGroupBox):
            ...
            try:
                ...
                for server in servers:
                    if server.get("id") == server_id:
                        db_manager.set_gpu_server_active(server.get("name", ""))
                        break
    ...
    def refresh_servers(self):
        ...
        try:
            ...
            for i, server in enumerate(servers):
                ...
                if active_server and server.get("id") == active_server.get("id"):
                    active_index = i
                    found_server = True
            ...
            if self._auto_connect_first_server and servers:
                if active_index >= 0:
                    ...
                    logger.info(
                        f"自动连接到已配置的活动GPU服务器: {servers[active_index].get('name', servers[active_index].get('host', '未知'))}"
                    )
                else:
                    # 没有活动服务器，选择第一个
                    ...
                    active_index = 0
                    logger.info(
                        f"自动连接到第一个可用的GPU服务器: {servers[0].get('name', servers[0].get('host', '未知'))}"
                    )
            ...
            if dialog.exec():
                try:
                    ...
                    QMessageBox.critical(
                        self, self.tr("error"), "添加GPU服务器失败：" + str(e)
                    )
        except Exception as e:
            logger.error("打开添加服务器对话框失败: " + str(e))
            QMessageBox.critical(
                self, self.tr("error"), "打开添加服务器对话框失败：" + str(e)
            )
    ...
    def _update_gpu_cards(self, stats):
        ...
        for i, gpu in enumerate(stats.gpus):
            ...
            if gpu["power_limit"] > 0:
                ...
                card["power_progress"].setFormat(
                    f"{int(gpu['power_usage'])}W ({power_percent:.1f}%)"
                )
                # 根据功率使用百分比设置进度条颜色
                self._set_progress_bar_color(card["power_progress"], power_percent)
            else:
                card["power_progress"].setMaximum(
                    100
                )  # 如果没有功率上限，使用默认的100
                card["power_progress"].setValue(int(gpu["power_usage"]))
        ...
        try:
            ...
            if self.gpu_selector.count() != stats.gpu_count:
                ...
                for i in range(stats.gpu_count):
                    gpu_name = (
                        stats.gpus[i]["info"] if i < len(stats.gpus) else f"GPU {i}"
                    )
    ...
    def _update_system_info(self, stats):
        ...
        if stats.network_io:
            self.network_recv_label_multi.setText(
                self._format_network_speed(recv_speed)
            )
            self.network_send_label_multi.setText(
                self._format_network_speed(send_speed)
            )
        else:
