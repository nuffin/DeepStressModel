...
class TestTab(QWidget):
    ...
    def start_test(self):
        ...
        try:
            ...
            success = self.test_executor.start_test(
                model_config["name"], tasks, test_task_id
            )
    ...
    def _on_progress_updated(self, progress: TestProgress):
        ...
        try:
            ...
            # 更新状态标签
            status_text = (
                self.tr("completed") + ": " + str(completed) + "/" + str(total)
            )
            ...
            # 添加平均响应时间
            detail_text += (
                self.tr("avg_response_time") + f": {progress.avg_response_time:.2f}s\n"
            )

            # 添加平均生成速度
            detail_text += (
                self.tr("avg_generation_speed")
                + f": {progress.avg_generation_speed:.1f}字/秒\n"
            )

            # 添加当前速度
            detail_text += (
                self.tr("current_speed") + f": {progress.current_speed:.1f}字/秒\n"
            )
        ...
        try:
            ...
            if response.success:
                ...
                if dataset_stats["successful"] > 0:
                    ...
                    dataset_stats["total_time"] = (
                        current_time - dataset_stats["start_time"]
                    )

                    if dataset_stats["total_time"] > 0:
                        # 考虑并发数计算平均生成速度
                        dataset_stats["avg_generation_speed"] = (
                            dataset_stats["total_chars"]
                            / dataset_stats["total_time"]
                ...
                current_records["total_time"] = (
                    current_time - current_records["start_time"]
                )
    ...
    def _on_api_call_mode_changed(self, button):
        ...
        logger.info(
            f"API调用方式已更改为: {'流式输出' if is_stream_mode else '直接输出'}"
        )
