...
class BenchmarkManager:
    ...
    def upload_dataset(self, file_path: str = None, api_key: str = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"离线数据包加载成功，版本: {dataset_version}，文件: {file_name}"
            )
            return True
        except Exception as e:
    ...
    def get_dataset_info(self) -> Dict[str, Any]:
        ...
        if offline_info:
            result = (
                self.dataset_info.copy()
                if self.dataset_info
                else {"version": "未知", "created_at": "", "size": 0}
            )
            # 合并离线数据集的详细信息
            result.update(
                {
    ...
    def _execute_test(
        self, test_data: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        执行测试
    ...
    def _collect_system_info(self) -> Dict[str, Any]:
        ...
        memory_info = {"total": memory.total, "available": memory.available}
        ...
        return {"gpu": gpu_info, "cpu": cpu_info, "memory": memory_info, "os": os_info}
