    ...
    def update(self, dataset_name: str, response: APIResponse):
        ...
        if response.success:
            ...
            if stats["successful"] > 0:
                ...
                stats["avg_generation_speed"] = (
                    stats["total_chars"] / stats["total_time"]
            ...
            total_time = sum(
                s["total_time"]
                for s in self.dataset_stats.values()
                if s["successful"] > 0
            )
...
class TestManager(QObject):
    ...
    def _update_progress(
        self, dataset_name: str, response: APIResponse, error_msg: str = ""
    ):
        """更新测试进度"""
        try:
            logger.debug(
                f"[DEBUG] 开始更新测试进度: dataset={dataset_name}, success={response.success}"
            )
            ...
            if response.success:
                logger.debug(
                    f"[DEBUG] 成功任务: dataset={dataset_name}, duration={response.duration:.2f}s"
                )
                ...
                if stats["successful"] > 0:
                    ...
                    logger.debug(
                        f"[DEBUG] 数据集 {dataset_name} 统计更新: avg_time={stats['avg_response_time']:.2f}s, speed={stats['avg_generation_speed']:.1f}字符/秒"
                    )
                ...
                if n > 0:
                    self.progress.avg_response_time = (
                        self.progress.avg_response_time * (n - 1) + response.duration
                    ) / n
                    self.progress.avg_generation_speed = (
                        self.progress.avg_generation_speed * (n - 1)
                        + response.generation_speed
                    ) / n
                    self.progress.avg_tps = (
                        self.progress.avg_tps * (n - 1)
                        + response.total_tokens / response.duration
                    ) / n
                logger.debug(
                    f"[DEBUG] 总体统计更新: avg_time={self.progress.avg_response_time:.2f}s, speed={self.progress.avg_generation_speed:.1f}字符/秒"
                )
            else:
                logger.debug(
                    f"[DEBUG] 失败任务: dataset={dataset_name}, error={error_msg or response.error_msg}"
                )
    ...
    async def _worker(
        ...
        try:
            while True:
                ...
                if task is None:
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        f.write(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {worker_id} 收到停止信号\n"
                        )
                ...
                try:
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        f.write(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {worker_id} 开始处理任务:\n"
                        )
                        ...
                        f.write(
                            f"- API调用方式: {'流式输出' if is_stream_mode else '直接输出'}\n"
                        )
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        f.write(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {worker_id} 完成任务:\n"
                        )
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        f.write(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {worker_id} 任务处理失败: {e}\n\n"
                        )
                    await result_queue.put(
                        (dataset_name, APIResponse(success=False, error_msg=str(e)))
                    )
                finally:
                    task_queue.task_done()
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 工作协程 {worker_id} 异常退出: {e}\n"
                )
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 工作协程 {worker_id} 结束\n"
                )
    ...
    async def _result_handler(
        ...
        try:
            while True:
                ...
                if result is None:
                    ...
                    if log_file:
                        with open(log_file, "a", encoding="utf-8") as f:
                            f.write(
                                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 结果处理协程收到停止信号\n"
                            )
                ...
                logger.debug(
                    f"[DEBUG] 发送结果接收信号: dataset={dataset_name}, success={response.success}"
                )
                ...
                if log_file:
                    with open(log_file, "a", encoding="utf-8") as f:
                        ...
                        f.write(
                            f"- 平均生成速度: {progress.avg_generation_speed:.2f}字/秒\n\n"
                        )
            ...
            if log_file:
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 结果处理协程异常退出: {e}\n"
                    )
                    f.write(traceback.format_exc() + "\n")
        finally:
            ...
            if log_file:
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 结果处理协程结束\n"
                    )
    ...
    async def run_test(
        ...
        try:
            ...
            logger.info(
                f"[DEBUG] 总权重: {total_weight}, 总并发数: {total_concurrency}"
            )
            ...
            for task in tasks:
                ...
                selected_prompts = random.sample(
                    task.prompts, min(task.concurrency, len(task.prompts))
                )
                ...
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 添加数据集任务: {task.dataset_name}\n"
                    )
            ...
            for i in range(total_concurrency):
                worker = asyncio.create_task(
                    self._worker(
                        f"worker_{i}", task_queue, result_queue, api_client, log_file
                    )
                )
                ...
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 工作协程 worker_{i} 启动\n"
                    )

            # 创建结果处理协程
            result_handler = asyncio.create_task(
                self._result_handler(
                    result_queue, self.progress, progress_callback, log_file
                )
            )
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                ...
                f.write(
                    f"平均生成速度: {self.progress.avg_generation_speed:.2f}字/秒\n"
                )
