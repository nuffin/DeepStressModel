...
def main():
    ...
    for dataset_name, dataset_stats in datasets.items():
        dataset_total_chars = dataset_stats.get(
            "total_chars", dataset_stats.get("total_bytes", 0)
        )
        dataset_total_time = dataset_stats.get("total_time", 0)
        dataset_real_char_speed = (
            dataset_total_chars / dataset_total_time if dataset_total_time > 0 else 0
        )
        dataset_avg_gen_speed = dataset_stats.get(
            "avg_gen_speed", dataset_stats.get("avg_generation_speed", 0)
        )
        ...
        print(
            f"差异: {abs(dataset_real_char_speed - dataset_avg_gen_speed):.2f}字符/秒"
        )
