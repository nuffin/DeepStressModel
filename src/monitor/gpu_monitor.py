    ...
    def _connect(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"连接远程服务器失败 (尝试 {attempt + 1}/{self.max_retries}): {e}"
                )
    ...
    def _execute_command(self, command: str) -> Optional[str]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"执行远程命令失败 (尝试 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt < self.max_retries - 1:
                    # 重置连接
                    try:
        ...
        try:
            ...
            if len(net_output) >= 2:
                ...
                if self._last_net_stats:
                    ...
                    if time_diff > 0:
                        if (
                            isinstance(self._last_net_stats, dict)
                            and "receive" in self._last_net_stats
                            and "transmit" in self._last_net_stats
                        ):
                            last_receive = self._last_net_stats["receive"]
                            last_send = self._last_net_stats["transmit"]

                            receive_rate = max(
                                0.1, (net_receive - last_receive) / time_diff * 1000
                            )  # KB/s
                            send_rate = max(
                                0.1, (net_send - last_send) / time_diff * 1000
                            )  # KB/s
                        else:
                            receive_rate = 0.1
                            send_rate = 0.1
    ...
    def init_monitor(self):
        ...
        try:
            ...
            if server:
                ...
                logger.info(
                    f"GPU监控器初始化成功: {server['host']}:{server.get('port', 22)}"
                )
            else:
